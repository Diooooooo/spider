    ...
    def parse_detail_info(self, response):
        for r in response.xpath('//div[@class="ltab_hd lmb3 clearfix"]/a'):
            if r.xpath('@href').extract_first() != 'javascript:void(0);':
                yield Request(response.urljoin(r.xpath('@href').extract_first()), self.parse_season_history_tab, dont_filter=True)
    ...
    def parse_season_history(self, response):
        if response.xpath('//ul[@id="match_group"]'):
            ...
            for s in response.xpath('//ul[@class="lsaiguo_round_list clearfix"]/li'):
                ...
                for t in infoJson:
                    yield Request(response.urljoin('http://liansai.500.com/team/%s/teamlineup/'%t['hid']), self.parse_sportsman_iteration, dont_filter=True)
                    yield Request(response.urljoin('http://liansai.500.com/team/%s/teamlineup/'%t['gid']), self.parse_sportsman_iteration, dont_filter=True)
        elif response.xpath('//div[@id="season_match_list"]'):
            for s in response.xpath('//tbody[@id="match_list_tbody"]/tr'):
                yield Request(response.urljoin('http://liansai.500.com%steamlineup/'%s.xpath('td[@class="td_lteam"]/a/@href').extract_first()), self.parse_sportsman_iteration, dont_filter=True)
                yield Request(response.urljoin('http://liansai.500.com%steamlineup/'%s.xpath('td[@class="td_rteam"]/a/@href').extract_first()), self.parse_sportsman_iteration, dont_filter=True)
        else:
            for s in response.xpath('//div[@class="lmb3"]'):
                for t in s.xpath('table/tbody/tr'):
                    yield Request(response.urljoin('http://liansai.500.com%steamlineup/'%t.xpath('td[@class="td_lteam"]/a/@href').extract_first()), self.parse_sportsman_iteration, dont_filter=True)
                    yield Request(response.urljoin('http://liansai.500.com%steamlineup/'%t.xpath('td[@class="td_rteam"]/a/@href').extract_first()), self.parse_sportsman_iteration, dont_filter=True)
