...
class FinalSpiderPipeline(object):
    ...
    def _conditional_league(self, tx, item):
        tx.execute('INSERT INTO qsr_league(country_id ,lea_name ,lea_full_name, lea_fid) '
                   ...
                   'LEFT JOIN qsr_league_country c ON c.country_name = i.country '
                   'ON DUPLICATE KEY UPDATE country_id = IFNULL(c.country_id, 0), lea_name = i.league, '
    ...
    def _conditional_team(self, tx, item):
        ...
        insertInto = "INSERT INTO qsr_team (team_name,team_name_en,team_icon,team_createdate," \
                     ...
                     "ON DUPLICATE KEY UPDATE team_home = i.team_home, team_name = i.team_name, team_name_en = i.team_name_en," \
                     "team_icon = i.team_icon_file_id, team_createdate = i.team_createdate, team_home_count= i.team_home_count, " \
    ...
    def _conditional_event(self, tx, item):
        if 'player-change' == item['type']:
            ...
            insertInto = "INSERT INTO qsr_team_season_event(season_id, team_id, sportsman_id_in, start_time, type_id, " \
                         "sportsman_name_in, sportsman_id_out, sportsman_name_out) " \
                         ...
                         "IFNULL(ts2.sports_name, i.sportsman_name_out) " \
            ...
            insertIntoIn = "INSERT INTO qsr_team_season_event(season_id, team_id, sportsman_id_in, start_time, " \
                           "type_id, sportsman_name_in) " \
                           "SELECT s.season_id, IF(i.is_home = 1, s.season_team_a, s.season_team_b), " \
                           "IFNULL(ts.sports_id, 0), i.start_time, t.type_id, " \
                           "IFNULL(ts.sports_name, i.sportsman_name_in) " \
    ...
    def _conditional_lottery(self, tx, item):
        insertInto = 'INSERT INTO qsr_team_season_sporttery(fid,sporttery_issue,league_id,season_id, status_id, type_id) ' \
                     ...
                     '"' + item['league'] + '" AS league, "' + item['home'] + '" AS home, "' \
                     + item['customer'] + '" AS customer, "' + item['date'] + '" AS _date, "' \
                     + item['time'] + '" AS _time, "' + str(item['result']) + '" AS status_id, "' + str(item['type_id']) + '" AS type_id) i ' \
                     ...
                     'ON DUPLICATE KEY UPDATE fid = i.fid, sporttery_issue = i.sporttery_issue, league_id = l.lea_id, '\
    ...
    def _conditional_news(self, tx, item):
        insertInto = 'INSERT INTO qsr_team_season_news (news_title ,news_content ,news_detail ,news_provenance_url ,' \
                     'news_provenance ,editor_create, news_conver) VALUES ("'+ item['title'] +'", "'+ item['title'] +'", "'+\
                     item['content'] +'", "' + item['url'] + '", "'+item['source']+'", "'+item['time']+'", "'+item['img']+'")'
        tx.execute(insertInto)
